...
{
   // OData 4, minus lambda expressions

   // Note that if you want to pass the resulting expressions to EF, you need to reference XPression.EF
   // and use post compiler extension method .AsLinqToEntities(). This method substitutes
   // all non-EF methods with EF methods
   // or use from XPression.EF IQueryable extension method Where("your query")

   // an alternative could be CompactSyntax, which provides the same functions as OData, but with another more compact syntax
   // example:
   // OData                                                    Compact
   // startswith(Name,'John')                                  Name^John
   // startswith(Name,'John') and contains(Name,'e')           Name^John&Name?e
   public class ODataSyntax<TExtender> : BaseSyntax<TExtender> where TExtender : new()
   {

      public static readonly ODataSyntax<TExtender> Instance = new ODataSyntax<TExtender>();
