...
{
   [TestFixture]
   public class UrlSyntaxTest
   {
      private readonly Parser _parser = new Parser(new Grammar(new UrlSyntax()){Strict = false});
      ...
      public void MethodsWork()
      {
         ...
         _p("CompanyName~ct~freds", c);
         _p("endswith(CompanyName,'Futterkiste')", c);
         _p("companyname~ew~Futterkiste", c);
         _p("startswith(CompanyName,'Alfr')", c);
         _p("companyName~sw~Alfr", c);
         _p("companyName~sw~'Alfr'", c);
         ...
         _p("cast(ShipCountry,String) eq ShipCountry", c);
         _p("isof(ShipCountry,String)",c);
